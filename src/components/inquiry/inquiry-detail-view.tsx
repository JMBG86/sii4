...
import { ChevronLeft, Loader2, MapPin, Save } from 'lucide-react'
...
import { updateInquiryLocation } from '@/app/(sii)/inqueritos/actions'
import CrimeLocationMap from '@/components/maps/map-wrapper'
import { useToast } from '@/components/ui/use-toast'

interface InquiryDetailViewProps {
    inquiryId: string
    onClose?: () => void // Optional callback for closing modal or navigation
    hideBack?: boolean // Option to hide back button
    readOnly?: boolean // Hide editing actions
}

export default function InquiryDetailView({ inquiryId, onClose, hideBack = false, readOnly = false }: InquiryDetailViewProps) {
    ...
    const handleSaveLocation = async () => {
        if (!currentLoc && (!inquiry?.latitude)) return
        if (!currentLoc) return

        setSavingLocation(true)
        try {
            await updateInquiryLocation(inquiry.id, currentLoc.lat, currentLoc.lng)
            toast({ title: "Localização guardada", description: "O local do crime foi atualizado." })
            loadData()
        } catch (e) {
            toast({ title: "Erro", description: "Falha ao guardar localização.", variant: "destructive" })
        } finally {
                    ...
                    <Card>
                        <CardHeader className="flex flex-row items-center justify-between pb-2">
                            <CardTitle className="text-base font-bold flex items-center gap-2">
                                ...
                                Localização Criminal
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <CrimeLocationMap
                                initialLat={inquiry.latitude}
                                initialLng={inquiry.longitude}
                                onLocationChange={(lat, lng) => {
                                    if (!readOnly) handleLocationChange(lat, lng)
                                }}
                            />
                            {!readOnly && (
                                <Button
                                ...
                                >
                                    {savingLocation ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
                                    Guardar Localização
                                </Button>
                            )}
                        </CardContent>
