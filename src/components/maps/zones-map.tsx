...
import { ProcessoDetailWrapper } from '@/app/sp/components/processo-detail-wrapper'
...
export default function ZonesMap({ points }: ZonesMapProps) {
    ...
    const [selectedSPProcessId, setSelectedSPProcessId] = useState<string | null>(null)
    const { toast } = useToast()

    // Always center on Albufeira
    const center = DEFAULT_CENTER

    const handleViewProcess = (point: CrimePoint) => {
        if (point.source === 'SP') {
            if (point.linkedInquiryId) {
                // If linked, show SII Detail
                setSelectedInquiryId(point.linkedInquiryId)
            } else {
                // If NOT linked, show SP Detail (Read-Only)
                setSelectedSPProcessId(point.id)
            }
        } else {
            // SII Logic (default)
            setSelectedInquiryId(point.id)
        }
    }
    ...
    return (
        <div>
            ...
            >
                ...
                {points.map((point) => {
                    ...
                    const SummaryContent = () => {
                        const isUnlinkedSP = point.source === 'SP' && !point.linkedInquiryId
                        const label = isUnlinkedSP ? 'Destino:' : 'Militar:'
                                ...
                                <div className="space-y-3 pb-1">
                                    ...
                                    <div className="pt-3 border-t flex justify-center">
                                        <Button
                                            ...
                                            onClick={() => handleViewProcess(point)}
                                        >
            ...
            {/* SP Detail Wrapper (Read-Only) */}
            <ProcessoDetailWrapper
                processoId={selectedSPProcessId}
                open={!!selectedSPProcessId}
                onOpenChange={(open) => !open && setSelectedSPProcessId(null)}
            />
        </div>
    )
}
